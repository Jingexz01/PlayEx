...
let PlayCommand = class PlayCommand extends BaseCommand_1.BaseCommand {
    async execute(message, args) {
        ...
        if (/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/.exec(url)) {
            try {
                ...
                if (skippedVideos !== 0) {
                    message.channel.send(createEmbed_1.createEmbed("warn", `${skippedVideos} ${skippedVideos >= 2 ? `videos` : `video`} are skipped because it's a private video`)
                    .setTimestamp()).catch(e => this.client.logger.error("PLAY_CMD_ERR:", e));
                }
    ...
    async handleVideo(video, message, voiceChannel, playlist = false) {
        ...
        if ((_a = message.guild) === null || _a === void 0 ? void 0 : _a.queue) {
            if (!this.client.config.allowDuplicate && message.guild.queue.songs.find(s => s.id === song.id)) {
                return message.channel.send(createEmbed_1.createEmbed("warn", `Song: **[${song.title}](${song.id})** is already queued, and this bot configuration disallow duplicated song(s) in the queue, ` +
            ...
            if (playlist)
                return;
            message.channel.send(createEmbed_1.createEmbed("info", `✅  **|**  **[${song.title}](${song.url})** has been added to the queue!`).setThumbnail(song.thumbnail).setTimestamp())
                .catch(e => this.client.logger.error("PLAY_CMD_ERR:", e));
        }
        else {
    ...
    async play(guild) {
        ...
        if (!song) {
            (_a = serverQueue.textChannel) === null || _a === void 0 ? void 0 : _a.send(createEmbed_1.createEmbed("info", `⏹  **|**  Queue has finished, use **\`${guild.client.config.prefix}play\`** again to play more songs!`).setTimestamp()).catch(e => this.client.logger.error("PLAY_ERR:", e));
        ...
        (_e = serverQueue.connection) === null || _e === void 0 ? void 0 : _e.play(songData.stream, { type: songData.canDemux ? "webm/opus" : "unknown", bitrate: "auto", highWaterMark: 1 }).on("start", () => {
            ...
            (_a = serverQueue.textChannel) === null || _a === void 0 ? void 0 : _a.send(createEmbed_1.createEmbed("info", `▶  **|**  Start playing: **[${song.title}](${song.url})**`).setThumbnail(song.thumbnail).setTimestamp()).catch(e => this.client.logger.error("PLAY_ERR:", e));
        }).on("finish", () => {
