...
let VolumeCommand = class VolumeCommand extends BaseCommand_1.BaseCommand {
    execute(message, args) {
        ...
        if (isNaN(volume))
            return message.channel.send(createEmbed_1.createEmbed("info", `🔊  **|**  The current volume is **\`${message.guild.queue.volume.toString()}\`**`).setTimestamp());
        if (volume < 0)
            volume = 0;
        if (volume === 0)
            return message.channel.send(createEmbed_1.createEmbed("warn", "Please pause the music instead of setting the volume to **\`0\`**").setTimestamp());
        if (Number(args[0]) > this.client.config.maxVolume) {
            return message.channel.send(createEmbed_1.createEmbed("warn", `You cannot set the volume above **\`${this.client.config.maxVolume}\`**`).setTimestamp());
        }
        ...
        message.channel.send(createEmbed_1.createEmbed("info", `🔊  **|**  Volume set to **\`${args[0]}\`**`).setTimestamp()).catch(console.error);
    }
};
tslib_1.__decorate([
