...
let QueueCommand = class QueueCommand extends BaseCommand_1.BaseCommand {
    execute(message) {
        ...
        if (Number((_d = (_c = message.guild) === null || _c === void 0 ? void 0 : _c.queue) === null || _d === void 0 ? void 0 : _d.songs.size) > 10) {
            ...
            let index = 0;
            embed.setDescription(`\`\`\`\n${indexes[index]}\`\`\``).setFooter(`• Page ${index + 1} of ${indexes.length}`, "https://hzmi.xyz/assets/images/390511462361202688.png").setTimestamp();
                ...
                msg.react("◀️").then(() => {
                    ...
                    msg.createReactionCollector((reaction, user) => reaction.emoji.name === "◀️" && user.id === message.author.id, { time: 80 * 1000 }).on("collect", () => {
                        if (index === 0)
                            return undefined;
                        index--;
                        embed.setDescription(indexes[index]).setFooter(`Page ${index + 1} of ${indexes.length}`, "https://hzmi.xyz/assets/images/390511462361202688.png").setTimestamp();
            ...
            message.channel.send(embed.setDescription(songs.join("\n")).setTimestamp()).catch(e => this.client.logger.error("QUEUE_CMD_ERR:", e));
        }
    }
